...
void PriorFactorRot2(const sym::Rot2<Scalar>& value, const sym::Rot2<Scalar>& prior,
  ...
  const Scalar _tmp2 = _prior[0] * _value[0] + _prior[1] * _value[1];
  const Scalar _tmp3 = _tmp2 + epsilon * ((((_tmp2) > 0) - ((_tmp2) < 0)) + Scalar(0.5));
  const Scalar _tmp4 = std::atan2(-_tmp0 + _tmp1, _tmp3);
  const Scalar _tmp5 = std::pow(Scalar(_tmp0 - _tmp1), Scalar(2));
  const Scalar _tmp6 = std::pow(_tmp3, Scalar(2));
  const Scalar _tmp7 = _tmp2 / _tmp3 + _tmp5 / _tmp6;
  const Scalar _tmp8 = _tmp5 + _tmp6;
  ...
  if (hessian != nullptr) {
    ...
    _hessian(0, 0) = _tmp10 * std::pow(_tmp3, Scalar(4)) * std::pow(_tmp7, Scalar(2)) /
                     std::pow(_tmp8, Scalar(2));
  }

  if (rhs != nullptr) {
    Eigen::Matrix<Scalar, 1, 1>& _rhs = (*rhs);

    _rhs(0, 0) = _tmp10 * _tmp4 * _tmp9;
  }
