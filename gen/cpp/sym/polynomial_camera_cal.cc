...
Eigen::Matrix<Scalar, 2, 1> PolynomialCameraCal<Scalar>::PixelFromCameraPointWithJacobians(
  ...
  const Scalar _tmp7 = _self[5] * _tmp6;
  const Scalar _tmp8 = [&]() {
    const Scalar base = _tmp6;
    return base * base * base;
  }();
  const Scalar _tmp9 = _self[7] * _tmp8;
  const Scalar _tmp10 = std::pow(_tmp6, Scalar(2));
  const Scalar _tmp11 = _self[6] * _tmp10;
  const Scalar _tmp12 =
      Scalar(1.0) * _tmp11 + Scalar(1.0) * _tmp7 + Scalar(1.0) * _tmp9 + Scalar(1.0);
  ...
  const Scalar _tmp18 = _tmp13 * point(0, 0);
  const Scalar _tmp19 =
      Scalar(1.0) * _tmp11 + Scalar(1.0) * _tmp7 + Scalar(1.0) * _tmp9 + Scalar(1.0);
  const Scalar _tmp20 = _tmp13 * point(1, 0);
  const Scalar _tmp21 = _self[0] * _tmp18;
  const Scalar _tmp22 = Scalar(1.0) * _tmp21;
  const Scalar _tmp23 = _self[1] * _tmp20;
  const Scalar _tmp24 = Scalar(1.0) * _tmp23;
  const Scalar _tmp25 = Scalar(2.0) * _self[5];
  const Scalar _tmp26 = _tmp3 * point(0, 0);
  const Scalar _tmp27 = _self[6] * _tmp6;
  const Scalar _tmp28 = Scalar(4.0) * _tmp27;
  const Scalar _tmp29 = _self[7] * _tmp10;
  const Scalar _tmp30 = Scalar(6.0) * _tmp29;
  const Scalar _tmp31 = _tmp25 * _tmp26 + _tmp26 * _tmp28 + _tmp26 * _tmp30;
  const Scalar _tmp32 = _tmp3 * point(1, 0);
  const Scalar _tmp33 = _tmp25 * _tmp32 + _tmp28 * _tmp32 + _tmp30 * _tmp32;
  const Scalar _tmp34 = _tmp17 * ((((_tmp1 - epsilon) > 0) - ((_tmp1 - epsilon) < 0)) + 1);
  const Scalar _tmp35 = (Scalar(1) / Scalar(2)) * _tmp12 * _tmp34;
  const Scalar _tmp36 = _tmp34 / [&]() {
    const Scalar base = _tmp2;
    return base * base * base;
  }();
  const Scalar _tmp37 = -_tmp0 * _tmp36 - _tmp36 * _tmp4;
  const Scalar _tmp38 = Scalar(1.0) * _self[5] * _tmp37 + Scalar(2.0) * _tmp27 * _tmp37 +
                        Scalar(3.0) * _tmp29 * _tmp37;

  // Output terms (4)
  Eigen::Matrix<Scalar, 2, 1> _pixel;

  _pixel(0, 0) = _self[2] + _tmp15 * point(0, 0);
  ...
  if (is_valid != nullptr) {
    ...
    _is_valid =
        std::max<Scalar>(0, std::min<Scalar>(_tmp17, (((_self[4] - std::sqrt(_tmp6)) > 0) -
  ...
  if (pixel_D_cal != nullptr) {
    ...
    _pixel_D_cal(0, 0) = _tmp18 * _tmp19;
    _pixel_D_cal(1, 0) = 0;
    _pixel_D_cal(0, 1) = 0;
    _pixel_D_cal(1, 1) = _tmp19 * _tmp20;
    _pixel_D_cal(0, 2) = 1;
    _pixel_D_cal(1, 2) = 0;
    _pixel_D_cal(0, 3) = 0;
    _pixel_D_cal(1, 3) = 1;
    _pixel_D_cal(0, 4) = _tmp22 * _tmp5;
    _pixel_D_cal(1, 4) = _tmp24 * _tmp5;
    _pixel_D_cal(0, 5) = _tmp10 * _tmp22;
    _pixel_D_cal(1, 5) = _tmp10 * _tmp24;
    _pixel_D_cal(0, 6) = _tmp22 * _tmp8;
    _pixel_D_cal(1, 6) = _tmp24 * _tmp8;
  }

  if (pixel_D_point != nullptr) {
    Eigen::Matrix<Scalar, 2, 3>& _pixel_D_point = (*pixel_D_point);

    _pixel_D_point(0, 0) = _tmp15 + _tmp21 * _tmp31;
    _pixel_D_point(1, 0) = _tmp23 * _tmp31;
    _pixel_D_point(0, 1) = _tmp21 * _tmp33;
    _pixel_D_point(1, 1) = _tmp16 + _tmp23 * _tmp33;
    _pixel_D_point(0, 2) = -_self[0] * _tmp26 * _tmp35 + _tmp21 * _tmp38;
    _pixel_D_point(1, 2) = -_self[1] * _tmp32 * _tmp35 + _tmp23 * _tmp38;
  }
